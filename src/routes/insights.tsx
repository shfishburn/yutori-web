...
function sentimentColor(sentiment: CoachInsight['sentiment']): string {
  switch (sentiment) {
    case 'positive':
      return 'text-success';
    case 'negative':
    ...
    <div className="rounded-2xl border border-edge bg-surface p-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <Icon
          ...
          className={`h-4 w-4 shrink-0 ${sentimentColor(insight.sentiment)}`}
          aria-hidden="true"
        />
      </div>
      <p className="mt-2 text-sm leading-relaxed text-fg-muted">{insight.body}</p>
      {insight.metric ? (
